...
public class Graph implements Iterable<Integer> {
    ...
    public List<Integer> shortestPath(int start, int stop) {
        class Vertex implements Comparable {
            ...
            public Vertex(int value, int distance, Vertex prev) {
                this.value = value;
            ...
            public int compareTo(Object v) {
                return this.distance - ((Vertex) v).distance;
            }
            @Override
            public String toString() {
                return this.value + " distance: " + distance + " prev: " + prev.value;
            }
        }
        ...
        Vertex v1 = new Vertex(start, 0, null);
        fringe.add(v1);
        ...
        while (!fringe.isEmpty()) {
            ...
            if (v.value == stop) {
                do {
                   pathStack.push(v.value);
                   v = v.prev;
                } while (v != null);
                break;
            }

            for (int n : neighbors(v.value)) {
                if (!visited.contains(n)) {
                    Edge e = getEdge(v.value, n);
                    int distToNextPoint = v.distance + e.weight;
                    Vertex next = new Vertex(n, distToNextPoint, v);
                    fringe.add(next);
                }
            }
    ...
    public Edge getEdge(int u, int v) {
        LinkedList<Edge> list = this.adjLists[u];
        for (Edge edge : list) {
            if (edge.to == v) {
                return edge;
            }
...
//        g2.printTopologicalSort();
        ...
        g5.generateG5();
        System.out.println(g5.shortestPath(0, 6)); // [0, 1, 4, 6]
        System.out.println(g5.shortestPath(0, 5)); // [0, 1, 4, 5]
        System.out.println(g5.shortestPath(0, 4)); // [0, 1, 4]
        System.out.println(g5.shortestPath(0, 3)); // [0, 1, 4, 6, 3]
        System.out.println(g5.shortestPath(0, 2)); // [0, 2]
        System.out.println(g5.shortestPath(0, 1)); // [0, 1]
    }
}
